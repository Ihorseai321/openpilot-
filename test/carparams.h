...
public:
    ...
    float rotationalInertia = CivicParams::ROTATIONAL_INERTIA * mass * wheelbase * wheelbase / (CivicParams::MASS * CivicParams::WHEELBASE * CivicParams::WHEELBASE);

    // TODO: start from empirically derived lateral slip stiffness for the civic and scale by
    
    // mass and CG position, so all cars will have approximately similar dyn behaviors
    float center_to_rear = wheelbase - centerToFront;
    float tireStiffnessFront = (CivicParams::TIRE_STIFFNESS_FRONT * tire_stiffness_factor) * mass / CivicParams::MASS * (center_to_rear / wheelbase) / (CivicParams::CENTER_TO_REAR / CivicParams::WHEELBASE);
    float tireStiffnessRear = (CivicParams::TIRE_STIFFNESS_REAR * tire_stiffness_factor) * mass / CivicParams::MASS * (centerToFront / wheelbase) / (CivicParams::CENTER_TO_FRONT / CivicParams::WHEELBASE);
