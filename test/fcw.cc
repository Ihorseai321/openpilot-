...
bool FCWChecker::update(log_t mpc_solution, float cur_time, bool active, float v_ego, float a_ego, float x_lead, float v_lead, float a_lead, float y_lead, float vlat_lead, bool fcw_lead, bool blinkers)
{
    ...
    if(fcw_lead){
      ...
      counters[4] = abs(y_lead) < 1.0 ? counters[4] + 1 : 0;
      counters[5] = abs(vlat_lead) < 0.4 ? counters[5] + 1 : 0;
