...
{
    float a_cruise_min = interp(v_ego, _A_CRUISE_MIN_BP, _A_CRUISE_MIN_V, 5);

    if(following){
        float a_cruise_max = interp(v_ego, _A_CRUISE_MAX_BP, _A_CRUISE_MAX_V_FOLLOWING, 4);
    }
    else{
        float a_cruise_max = interp(v_ego, _A_CRUISE_MAX_BP, _A_CRUISE_MAX_V, 4);
    }
...
void limit_accel_in_turns(float v_ego, float angle_steers, float a_target[], CarParams CP, float ret[])
{
    ...
    float a_x_allowed = sqrt(std::max(a_total_max * a_total_max - a_y * a_y, 0.0));
...
{
    if(enabled){
        std::string PlanSource[4] = {"model", "cruise", "mpc1", "mpc2"};
        ...
        if(mpc1.prev_lead_status){
            solutions[3] = mpc1.v_mpc;
        }
        if(mpc2.prev_lead_status){
            solutions[4] = mpc2.v_mpc;
        }
...
void Planner::update(Handler handler, PubSocket *plan_sock, PubSocket *livelongitudinalmpc_sock)
{
    ...
    long_control_state = handler.long_control_state;
    v_cruise_kph = handler.v_cruise_kph;
    force_slow_decel = handler.force_slow_decel;
    v_cruise_setpoint = v_cruise_kph * KPH_TO_MS;
    ...
    if(handler.p_has_poly){
        ...
        for(int i = 0; i < 192; ++i){
            if(abs(curv[i]) < 1e-4){
                ...
                v_curvature = sqrt(a_y_max / abs(curv[i]));
            }
            min_v_curcature = min_v_curcature < v_curvature ? min_v_curcature : v_curvature;
        }
        model_speed = min_v_curcature;
        model_speed = std::max(20.0 * MPH_TO_MS, model_speed); // Don't slow down below 20mph
    }
    else{
    ...
    if(enabled && !first_loop){
      ...
      jerk_limits[0] = std::min(-0.1, accel_limits[0]);
      jerk_limits[1] = std::max(0.1, accel_limits[1]); // TODO: make a separate lookup for jerk tuning
      ...
      bool starting = (long_control_state == LongCtrlState.starting);
