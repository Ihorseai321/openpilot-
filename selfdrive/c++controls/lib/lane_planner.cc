...
void model_polyfit(float points[50], float path_pinv[4][50], float ret[4])
{
    float sum;
    for(int i = 0; i < 4; ++i){
        sum = 0.0;
...
void LanePlanner::parse_model(Handler handler)
{
    ...
    if(handler.has_meta_desirePrediction){
        l_lane_change_prob = handler.meta_desirePrediction[2];
        r_lane_change_prob = handler.meta_desirePrediction[3];
    }
}
