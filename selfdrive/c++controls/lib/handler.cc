...
Handler::Handler()
{
  ...
  prev_update_time = -1;
  car_state_time = 0;
  controls_state_time = 0;
  radar_state_time = 0;
  model_time = 0;
  liveParameters_time = 0;
...
void Handler::handle_car_state(cereal::CarState::Reader car_state, unsigned long current_time)
{
...
void Handler::handle_controls_state(cereal::ControlsState::Reader controls_state, unsigned long current_time) {
  ...
  curvature = controls_state.getCurvature();
  controls_state_time = current_time;
}

void Handler::handle_radar_state(cereal::RadarState::Reader radar_state, unsigned long current_time, unsigned long rcv_time)
{
  ...
  radar_rcv_time = rcv_time;
}

void Handler::handle_model(cereal::ModelData::Reader model, unsigned long current_time)
{
...
void Handler::handle_live_parameters(cereal::LiveParametersData::Reader live_parameters, unsigned long current_time)
{
  angleOffset = live_parameters.getAngleOffset();
  stiffnessFactor = live_parameters.getStiffnessFactor();
  steerRatio = live_parameters.getSteerRatio();
  angleOffsetAverage = live_parameters.getAngleOffsetAverage();
  ...
  liveParameters_time = current_time;
}
...
void Handler::handle_log(cereal::Event::Reader event) {
  ...
  case cereal::Event::RADAR_STATE:
    handle_radar_state(event.getRadarState(), current_time, rcv_time);
    break;
