...
void CarState::update(Parser &cp)
{
  ...
  cout << "---->before Kalman filter" << endl;
  // Kalman filter
  ...
  cout << "---->after Kalman filter1" << endl;
  ...
  cout << "---->after Kalman filter2" << endl;
  ...
  cout << "---->after Kalman filter3" << endl;
  std::pair<std::string, std::string> pcm_acc_status_pair("Cruise_Status", "Cruise_State");
  pcm_acc_status = (bool)cp.vl[pcm_acc_status_pair];
  cout << "---->after Kalman filter4" << endl;
  ...
  cout << "---->after Kalman filter5" << endl;
  ...
  cout << "---->after Kalman filter6" << endl;
  ...
  cout << "---->after Kalman filter9" << endl;
  cout << "---->after Kalman filter9" << endl;

  cout << "CarState.v_ego->" << v_ego << endl;
  cout << "CarState.v_ego_raw->" << v_ego_raw << endl;
  cout << "CarState.standstill->" << standstill << endl;
  cout << "CarState.v_wheel_fl->" << v_wheel_fl << endl;
  cout << "CarState.v_wheel_rl->" << v_wheel_rl << endl;
}