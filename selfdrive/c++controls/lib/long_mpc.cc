#include <cmath>
#include "long_mpc.h"
#include "longitudinal_mpc/longitudinal_mpc.h"
...
void LongitudinalMpc::set_cur_state(float v, float a)
{
    cur_state.v_ego = v;
    cur_state.a_ego = a;
}
...
void LongitudinalMpc::update(Handler handler, PubSocket *livelongitudinalmpc_sock, LeadData lead)
{
    ...
    if(lead.status){
        ...
        if(!prev_lead_status || std::abs(x_lead - prev_lead_x) > 2.5){
            long_init_with_simulation(v_mpc, x_lead, v_lead, a_lead, a_lead_tau);
            new_lead = true;
        }
    ...
    int n_its = long_run_mpc(&cur_state, &mpc_solution, a_lead_tau, a_lead);
